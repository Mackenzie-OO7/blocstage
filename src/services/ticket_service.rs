...
use chrono::Utc;
use rust_decimal::prelude::{Zero, Signed};
...
use printpdf::{PdfDocument, Mm, Point, Rgb};
use std::path::Path;
...
impl TicketService {
    ...
    pub async fn purchase_ticket(&self, ticket_type_id: Uuid, user_id: Uuid) -> Result<Ticket> {
        ...
        let ticket_type = self.get_ticket_type_with_lock(&mut tx, ticket_type_id).await?;
        ...
        let qr_data = format!("ticket:{}:{}:{}", ticket_type_id, user_id, Utc::now().timestamp());
        let qr_code = self.generate_qr_code(&qr_data)?;
        
        let ticket = self.create_ticket_in_transaction(&mut tx, ticket_type_id, user_id, Some(qr_code)).await?;
        
        if let Some(price) = ticket_type.price.clone() {
            if !price.is_zero() {
                let transaction = self.create_transaction_in_transaction(
                    &mut tx,
                    ticket.id,
                    user_id,
                    price,
                ...
                let payment_result = self.process_payment(&user, &ticket_type, &transaction).await;
                
                match payment_result {
                    Ok(tx_hash) => {
                        // Update transaction with Stellar hash
                        self.update_transaction_hash_in_transaction(&mut tx, &transaction, &tx_hash).await?;
                        self.update_transaction_status_in_transaction(&mut tx, &transaction, "completed").await?;
                        
                        self.decrease_remaining_in_transaction(&mut tx, &ticket_type).await?;
                    },
                    Err(e) => {
                        self.update_transaction_status_in_transaction(&mut tx, &transaction, "failed").await?;
                        
                        self.update_ticket_status_in_transaction(&mut tx, &ticket, "cancelled").await?;
        ...
        tokio::spawn(async move {
            match self.generate_pdf_ticket(ticket_id).await {
                Ok(_) => info!("PDF ticket generated for ticket: {}", ticket_id),
    ...
    async fn get_ticket_type_with_lock<'a>(&self, tx: &mut SqlxTransaction<'a, Postgres>, id: Uuid) -> Result<TicketType> {
        let row = sqlx::query!(
    ...
    async fn process_payment(&self, user: &User, ticket_type: &TicketType, transaction: &Transaction) -> Result<String> {
        ...
        let user_secret_key = user.stellar_secret_key.clone()
            .ok_or_else(|| anyhow!("User has no Stellar wallet"))?;
        
        let amount = transaction.amount.to_string();
        let tx_hash = self.stellar_service.send_payment(&user_secret_key, &platform_wallet, &amount).await?;
    ...
    pub async fn verify_ticket(&self, ticket_id: Uuid) -> Result<bool> {
        ...
        if let Some(nft_id) = &ticket.nft_identifier {
            let owner = User::find_by_id(&self.pool, ticket.owner_id).await?
            ...
            if let Some(public_key) = &owner.stellar_public_key {
                let is_valid = self.stellar_service.verify_nft_ownership(
                    public_key, 
                    nft_id, 
                    &issuer_public_key
                ).await?;
    ...
    pub async fn generate_pdf_ticket(&self, ticket_id: Uuid) -> Result<String> {
        ...
        let ticket_type = TicketType::find_by_id(&self.pool, ticket.ticket_type_id).await?
            .ok_or_else(|| anyhow!("Ticket type not found"))?;
        
        let event = Event::find_by_id(&self.pool, ticket_type.event_id).await?
        ...
        self.send_ticket_email(&owner.email, &pdf_url, &event.title).await?;
    ...
    async fn upload_to_storage(&self, path: &str, content: Vec<u8>) -> Result<String> {
        if let Some(client) = &self.s3_client {
            let bucket_name = env::var("S3_BUCKET_NAME")
                .map_err(|_| anyhow!("S3_BUCKET_NAME not set"))?;
            ...
            let base_url = env::var("S3_BASE_URL")
                .map_err(|_| anyhow!("S3_BASE_URL not set"))?;
            
            return Ok(format!("{}/{}/{}", base_url, bucket_name, path));
        } else {
            let storage_dir = env::var("LOCAL_STORAGE_DIR")
    ...
    async fn send_ticket_email(&self, email: &str, pdf_url: &str, event_title: &str) -> Result<()> {
        ...
        let email = Message::builder()
            .from(env::var("EMAIL_FROM").unwrap_or_else(|_| "tickets@example.com".to_string()).parse()?)
            .to(email.parse()?)
    ...
    pub async fn convert_to_nft(&self, ticket_id: Uuid) -> Result<Ticket> {
        ...
        let user = User::find_by_id(&self.pool, ticket.owner_id).await?
            .ok_or_else(|| anyhow!("User not found"))?;
        
        let issuer_secret_key = env::var("NFT_ISSUER_SECRET_KEY")
            .map_err(|_| anyhow!("NFT issuer not configured"))?;
        
        let asset_code = format!("TKT{}", ticket.id.to_string().replace("-", "").chars().take(12).collect::<String>());
        
        let user_public_key = user.stellar_public_key.clone()
            .ok_or_else(|| anyhow!("User has no Stellar wallet"))?;
        
        let tx_hash = self.stellar_service.issue_nft_asset(&issuer_secret_key, &asset_code, &user_public_key).await?;
    ...
    pub async fn transfer_ticket(&self, ticket_id: Uuid, from_user_id: Uuid, to_user_id: Uuid) -> Result<Ticket> {
        ...
        // if it's an NFT ticket, process it on the blockchain
        if let Some(nft_id) = &ticket.nft_identifier {
        ...
        tokio::spawn(async move {
            match service.generate_pdf_ticket(tid).await {
                Ok(_) => info!("PDF ticket regenerated after transfer for ticket: {}", tid),
    ...
    pub async fn cancel_ticket(&self, ticket_id: Uuid, user_id: Uuid) -> Result<Ticket> {
        ...
        if ticket.status != "valid" {
            return Err(anyhow!("Ticket cannot be cancelled (status: {})", ticket.status));
        }
        
        let updated_ticket = self.update_ticket_status_in_transaction(&mut tx, &ticket, "cancelled").await?;
        ...
        if let Some(tx_record) = transaction {
            if tx_record.status == "completed" && tx_record.amount.is_positive() {
                ...
                let refund_hash = self.stellar_service.process_refund(
                    &refund_secret_key,
                    &user_public_key,
                    &tx_record.amount.to_string()
                ).await?;
                
                tx_record.process_refund(&self.pool, None, Some("Ticket cancelled".to_string())).await?;
                tx_record.update_refund_hash(&self.pool, &refund_hash).await?;
            }
        }
    ...
    pub async fn get_user_tickets(&self, user_id: Uuid) -> Result<Vec<(Ticket, TicketType, Event)>> {
