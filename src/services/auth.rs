...
impl AuthService {
    ...
    pub async fn register(&self, user_req: CreateUserRequest) -> Result<User> {
        ...
        //     Ok(tx_hash) => {
        //         info!(
        //             "✅ USDC trustline for user {}: has been created! {}",
        //             user.id, tx_hash
        //         );
        //     }
        //     Err(e) => {
        //         warn!(
        //             "⚠️ Failed to auto-create USDC trustline for user {}: {}",
        //             user.id, e
        //         );
        //     }
        // }
    ...
    // TODO: this should be an auto create account fn that funds the account with 1xlm
    ...
    //     let sponsor_manager =
    //         crate::services::sponsor_manager::SponsorManager::new(self.pool.clone())
    //             .map_err(|e| anyhow!("😞 Failed to initialize sponsor manager: {}", e))?;

    //     let sponsor_info = sponsor_manager
    //         .get_available_sponsor()
    //         .await
    ...
    // REPLACE your existing reset_password method
    ...
    // Enhanced token verification - keep your existing method but add these security checks
    pub async fn verify_token(&self, token: &str) -> Result<Uuid> {
        ...
        // Your existing enhanced validation settings (keep these!)
        ...
        // Your existing additional security checks (keep these!)
        ...
        // Your existing token age validation (keep this!)
        ...
        // ➕ ADD: Redis session validation (ONLY NEW CODE)
        if let Some(redis) = &self.redis {
            match redis
                ...
                .await
            {
                ...
                Err(e) => {
                    ...
                    // Continue without session check if Redis fails
                }
            }
        }
    ...
    pub async fn logout(&self, user_id: Uuid, token: &str) -> Result<()> {
        if let Some(redis) = &self.redis {
            if let Ok(jwt_id) = self.extract_jwt_id(token) {
                // Use enhanced removal that cleans up metadata too
                redis
    ...
    // ADD: New logout all method (this is new functionality)
    pub async fn logout_all_sessions(&self, user_id: Uuid) -> Result<()> {
        if let Some(redis) = &self.redis {
            // Use enhanced invalidation that cleans up all metadata too
            redis
    ...
    // ADD: Helper method for extracting JWT ID
    ...
    async fn send_password_changed_email(&self, user: &User) -> Result<()> {
        let Some(email_service) = &self.email_service else {
            return Ok(()); // Gracefully skip if not available
        };
    ...
    async fn generate_token(
        ...
        // Enhanced Redis session tracking with metadata
        if let Some(redis) = &self.redis {
        ...
        // Your existing self-verification (now with await)
